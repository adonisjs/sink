...
function getHtml (css: string, moduleName: string, content: string): string {
  return `<html>
  <head>
 ...
 * process.
 */
export async function renderMarkdown (mdFilePath: string, moduleName: string) {
  try {
    const contents = await readFile(mdFilePath, 'utf-8')
    const html = getHtml(css, moduleName, marked(contents.trim()))
